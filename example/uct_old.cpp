...
//rave1
  float rave_ucb(Node* child, float explore_coeff){
    ...
    float alpha = min(1.0, 0.1 * log(stat.update_count()));
...
//rave2
  float rave_ucb(Node* child, float explore_coeff){
    float score = child->stat.ucb (child->player, explore_coeff);
    float rave_score = child->rave_stat.ucb (child->player, explore_coeff);
    float alpha = min(0.9, 0.01 * stat.update_count());
    float beta = 1.0 - alpha;
    return alpha*score + beta*rave_score;
  }
...
//rave3
  float rave_ucb(Node* child, float explore_coeff){
    float score = child->stat.ucb (child->player, explore_coeff);
    float rave_score = child->rave_stat.ucb (child->player, explore_coeff);
    float alpha = min(1.0, 0.00001 * stat.update_count() * stat.update_count());
    float beta = 1.0 - alpha;
    return alpha*score + beta*rave_score;
  }
*/
...
//rave4
  float rave_ucb(Node* child, float explore_coeff){
    float score = child->stat.ucb (child->player, explore_coeff);
    float rave_score = child->rave_stat.ucb (child->player, explore_coeff);
    float alpha = min(1.0, 0.001 * stat.update_count());
    float beta = 1.0 - alpha;
    return alpha*score + beta*rave_score;
  }
*/
  ...
  Node* find_uct_child (float explore_rate) {
    ...
    vertex_for_each_all(v) {
      ...
      float child_urgency = rave_ucb(child, explore_coeff);
...
public:
  ...
  void rave_update (Board& board, Board& base_board, float sample) {
    ...
    if (history_top == 0)
      ...
      parent = history_top - 1;
    
// Update all levels
//    reps (hist_i, 0, history_top - 1){

//Update only last level
    reps (hist_i, parent, parent + 1){

      //cout << hist_i << endl;
      node_for_each_child(history [hist_i], child,
        if (rave_node_in_playout(child, board, move_from, move_to))
...
public:
  ...
  Uct (Board& base_board_) : base_board (base_board_), policy(global_random) { 
    ...
    //rep (ii, base_board_.move_no){
    //  Vertex v = base_board_.move_history[ii].get_vertex();
    //  cout << v << endl;
    //}
  ...
  void do_playout (Player first_player){
    ...
    do {
      if (tree.act_node ()->no_children ()) { // we're finishing it
        ...
        tree.rave_update(play_board, base_board, 1 - score - score);
        return;
      }
